...
def test_offset_of_zero_with_id():
    ...
    with open("test/cassettes/test-pagination_params/test_offset_of_zero_with_id.yaml") as f:
        x = yaml.safe_load(f)
...
def test_offset_of_zero_without_id():
    ...
    assert 0 == res['message']['query']['start-index']
    with open("test/cassettes/test-pagination_params/test_offset_of_zero_without_id.yaml") as f:
        x = yaml.safe_load(f)
    uri = x['interactions'][0]['request']['uri']
    assert "offset" in uri
